...
export default class CSC extends React.Component {
    ...
    queryCard() {
        if (this.isDuoBao) {
            let url = window.location.href.replace('#/csc', '#/clienthelp');
            window.location = 'duobao://native/open?title=' + window.encodeURIComponent('卡密查询') + '&type=0&link=' + window.encodeURIComponent(url);
        } else {
            utils.downLoadClient();
        }
    }

    queryTelMoney() {
        if (this.isDuoBao) {
            let url = window.location.href.replace('#/csc', '#/queryphoto');
            window.location = 'duobao://native/open?title=' + window.encodeURIComponent('话费查询') + '&type=0&link=' + window.encodeURIComponent(url);
        } else {
            utils.downLoadClient();
        }
    }
    ...
    openContent(e) {
        ...
        if (ele.className == 'csc-title' || ele.parentElement.className == 'csc-title') {
            let id     = ele.id || ele.parentElement.id,
                ...
                idEle  = document.querySelector('#t' + id).style,
